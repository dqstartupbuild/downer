...
export function ScreenshotEditor() {
  const { state, setState, hydrated, savedAt, saveError, reset, undo, redo, canUndo, canRedo } = useProject();
  ...
  async function exportAll() {
    ...
    if (missingScreens.length > 0 || reusedBackScreens.length > 0) {
      const details = [
        missingScreens.length
          ? `${missingScreens.length} screen${missingScreens.length === 1 ? "" : "s"} will export with an empty window.`
          : null,
        reusedBackScreens.length
          ? `${reusedBackScreens.length} two-window screen${reusedBackScreens.length === 1 ? "" : "s"} will reuse the primary screenshot in back.`
          : null,
      ].filter(Boolean);
      ...
      for (const size of sizes) {
        for (let i = 0; i < currentSlides.length; i++) {
          ...
          try {
            ...
            const path = `${state.device}/${size.w}x${size.h}/${locale}/${filename}`;
    ...
    if (okCount > 0) {
      try {
        ...
        a.href = url;
        a.download = `${slugify(state.appName)}-${state.device}-${stamp()}.zip`;
        a.click();
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-background">
      ...
      <Toolbar
        ...
        onResetAll={() => {
          ...
          toast.success("Restored SortDock starter screens");
        }}
        onUndo={undo}
        onRedo={redo}
        canUndo={canUndo}
