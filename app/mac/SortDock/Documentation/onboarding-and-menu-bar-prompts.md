...
First launch keeps SortDock visible while the user selects a watched folder, adds a destination and keyword rule, chooses how moves are handled, and confirms the summary. Sorting and the login item do not begin before that finish action.
...
- `OnboardingView` reuses `SortDockStore` destination, bookmark, keyword-rule, and settings paths rather than a second persistence model.
